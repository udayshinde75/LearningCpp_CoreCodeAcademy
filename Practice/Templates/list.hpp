...
class node {
    friend class list<T>;
    friend std::ostream& operator<<(std::ostream& cout, const list<T>& list_object);
    private:
        T data;
        node* prev;
        node* next;
...
class list {
    public: 
        ...
        status_t insert_end(const T& data) {
            generic_insert(head_node->prev, new node<T>(data), head_node->next);
            return SUCCESS;
        }

        status_t insert_after(const T& e_data, const T& data) {
            node<T>* e_node = search_node(e_data);
            if (e_node == nullptr)
            {
                return LIST_INVALID_DATA;
            }
            generic_insert(e_data, new node<T>(data), e_node->next);
            return SUCCESS;
        }

        status_t insert_before(const T& e_data, const T& data) {
            node<T>* e_node = search_node(e_data);
            if (e_node == nullptr)
            {
                return LIST_INVALID_DATA;
            }
            generic_insert(e_node->prev, new node<T>(data), e_node);
            return SUCCESS;
        }

        status_t get_start(T& data) {
            if (empty())
            {
                return LIST_EMPTY;
            }
        ...
        status_t remove_data(T& data) {
            node<T> delete_node = search_node(data);
    ...
    private:
        ...
        node<T>* search_node(const T& data) {
            if (node<T>* run = head_node->next; run != head_node; run = run->next)
            {
                if (data == run->data)
                {
                    return run;
